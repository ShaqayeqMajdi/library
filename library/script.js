...
function DeleteFromCart(bookId) {
  const targetIndex = cartItems.indexOf(bookId);
  if (targetIndex != -1) cartItems.splice(targetIndex, 1);
  localStorage.setItem("cart", JSON.stringify(cartItems));
  render(BOOKS);
  updateCartCount();
}
...
function renderCartItems() {
  const filterBooks = BOOKS.filter((book) => cartItems.includes(book.id));
    ...
    cartUl.innerHTML = `
    <div class="empty-basket">  <img src="./assets/images/icon/basket.png" alt="" /> 
...
<span><button onclick= DeleteFromCart(${item.id}) class="delete-button">×</button></span>
</li>
`;
    })
    .join("");
...
function handleCartClick() {
  renderCartItems();
  cartUl.classList.toggle("hidden");
}
